...
export const createShoppingList = async (
  ...
  const existingLists = await ShoppingListModel.find({ name, user }).lean();
  if (existingLists && existingLists.length !== 0) {
    throw new ResourceAlreadyExistError({
      message:
        "List of that name already exist in saved shopping lists, Please provide a different name!",
    });
  }
...
export const updateList = async (
  ...
  if (
    "isFinished" in updates &&
    updatedDoc.items &&
    !checkAllItemsCompletedInList(updatedDoc.id)
  ) {
...
export const checkAllItemsCompletedInList = async (
  ...
  return (
    ...
    foundList.items.every((item) => item.completed === true)
  );
};
...
export const deleteItemFromList = async (
  ...
  const itemDocDeleted = await ItemModel.findOneAndDelete({
    _id: id,
    name,
    list,
  });
